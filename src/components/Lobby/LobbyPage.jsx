...
function LobbyPage() {
  ...
  useEffect(() => {
    ...
    const errorMsg = useMemo(() => {
        ...
        if (rooms.some((room) => room.id === roomNum) || startingVals.includes(roomNum.toLowerCase())) return "Room already exists";
    ...
    const joinErrorMsg = useMemo(() => {
        ...
        if (!rooms.some((room) => room.id === joinRoomNum)) return "Room Doesn't Exist";
        return "Must be at least 4 characters";
    }, [joinRoomNum]);

    const filterErrorMsg = useMemo(() => {
        const startingVals = ["Game 1", "Game 2", "Game 3", "Game 4"];
        const isFilteringInit = startingVals.some((val) => val.toLowerCase().includes(roomFilter.toLowerCase()));
        const doesRoomExist = rooms.some((room) => {
            return room.id.toLowerCase().includes(roomFilter.toLowerCase());
        });
        if (!doesRoomExist && !isFilteringInit && roomFilter.length) return "That room doesn't exist";
        return null;
    }, [roomFilter]);
                ...
                >
                    ...
                    <form style={{ textAlign: "right" }}>
                        <TextField
                            ...
                            value={roomNum}
                            helperText={errorMsg}
                            error={(hasClicked && roomNum.length < 4) || rooms.some((room) => room.id === roomNum.replace(/\s+/g, "-").toLowerCase())}
                            onChange={(e) => {
                                setHasClicked(false);
                        ...
                        <FormControlLabel
                            ...
                            control={<Checkbox checked={isPrivate} color="secondary" onChange={() => setIsPrivate((curr) => !curr)} />}
                            label="Private Game"
                        />
                        <Button
                            sx={{ marginTop: "5px" }}
                            ...
                            onClick={(e) => {
                                e.preventDefault();
                                setHasClicked(true);
                                if (roomNum.length > 3 && !rooms.some((room) => room === roomNum.replace(/\s+/g, "-").toLowerCase())) {
                                    socketRef.current.emit("create room", {
                                        id: roomNum.replace(/\s+/g, "-").toLowerCase(),
                                        isPrivate: isPrivate,
                                    });
                                    navigate(`/game-room/${roomNum.replace(/\s+/g, "-").toLowerCase()}`);
                                }
                            }}
                        >
                            Create Room
                ...
                >
                    ...
                    >
                        <TextField
                            ...
                            error={(hasClickedJoin && joinRoomNum.length < 4) || !rooms.some((room) => room.id === roomNum)}
                            onChange={(e) => {
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                ...
                                if (joinRoomNum.length > 4 && rooms.some((room) => room.id === joinRoomNum.replace(/\s+/g, "-").toLowerCase())) {
                                    navigate(`/game-room/${joinRoomNum.replace(/\s+/g, "-").toLowerCase()}`);
                                }
                            }}
                        >
                            Join Private Room
                ...
                >
                    ...
                    {rooms
                        ...
                        .map((val) => {
                            ...
                            if (starting.includes(val.id) || val.isPrivate) return <div key={val.id}></div>;
